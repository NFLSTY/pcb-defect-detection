...
class ConvNormAct(nn.Module):
    def __init__(self,
        ...
        self.norm_layer = None if norm_layer is None else norm_layer(out_channels)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, dim_head=None):
        ...
        weight_dim = self.num_heads * self.dim_head
        self.to_qvk = nn.Linear(dim, weight_dim * 3, bias=False)
        self.scale_factor = self.dim_head ** -0.5
        self.w_out = nn.Linear(weight_dim, dim, bias=False)
...
class MobileVitBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d_model, layers, mlp_dim):
        ...
        self.local_representation = nn.Sequential(
            # Encode local spatial information with a depth-wise convolution to capture local features while keeping computational cost low
            ConvNormAct(in_channels, in_channels, 3),
            # Projects the tensor to a high-diementional space to capture more complex local features
            ConvNormAct(in_channels, d_model, 1)
        )

        self.transformer = Transformer(d_model, layers, 1, 32, mlp_dim, 0.1)

        # Fusion block to combine local and global representations
...
# Model Part (MobileViT-XXS configuration)
model_cfg_xxs = {
...
def mobilevit_xxs(img_size=256, num_classes=1000):
    cfg = model_cfg_xxs
    return MobileViT(img_size, cfg["features"], cfg["d"], cfg["layers"], cfg["expansion_ratio"], num_classes)
